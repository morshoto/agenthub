...
func renderTerraformVars(vars terraformVars) string {
	...
	lines := []string{
		...
		fmt.Sprintf("%-*s = %s", maxWidth, "github_ssh_key_secret_arn", terraformQuoted(vars.GitHubSSHKeySecretARN)),
		...
		fmt.Sprintf("%-*s = %s", maxWidth, "security_group_name", terraformQuoted(vars.SecurityGroupName)),
